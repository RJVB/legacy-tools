...
#ifndef CALIB_SECONDS
#	warning "Using default calibration time of 15 seconds"
#	define CALIB_SECONDS	15.0
#endif
...
flop_t piref, sa, one, two,four,three;

flop_t A0 = 1.0;
flop_t A1 = -0.1666666666671334;
flop_t A2 = 0.833333333809067E-2;
flop_t A3 = 0.198412715551283E-3;
flop_t A4 = 0.27557589750762E-5;
flop_t A5 = 0.2507059876207E-7;
flop_t A6 = 0.164105986683E-9;

flop_t B0 = 1.0;
flop_t B1 = -0.4999999999982;
flop_t B2 = 0.4166666664651E-1;
flop_t B3 = -0.1388888805755E-2;
flop_t B4 = 0.24801428034E-4;
flop_t B5 = -0.2754213324E-6;
flop_t B6 = 0.20189405E-8;

flop_t C0 = 1.0;
flop_t C1 = 0.99999999668;
flop_t C2 = 0.49999995173;
flop_t C3 = 0.16666704243;
flop_t C4 = 0.4166685027E-1;
flop_t C5 = 0.832672635E-2;
flop_t C6 = 0.140836136E-2;
flop_t C7 = 0.17358267E-3;
flop_t C8 = 0.3931683E-4;

flop_t D1 = 0.3999999946405E-1;
flop_t D2 = 0.96E-3;
flop_t D3 = 0.1233153E-5;

flop_t E2 = 0.48E-3;
flop_t E3 = 0.411051E-6;

int main(int argc, char *argv[] )
{

#ifdef ROPT
   register flop_t s,u,v,w,x;
#else
   flop_t s,u,v,w,x;
#endif
   ...
   printf("\n");
#ifdef SINGLE_PREC
   printf("   FLOPS C Program (Single Precision), V2.0 18 Dec 1992\n\n");
#else
   printf("   FLOPS C Program (Double Precision), V2.0 18 Dec 1992\n\n");
#endif

#ifdef __GCCOPT__
	{ char buf[ 16 + strlen(argv[0]) ];
/* 		sprintf( buf, "ident %s", argv[0] );	*/
/* 		system( buf );	*/
		fprintf( stdout, "%s: %s\n", argv[0], ident_stub() );
	}
	fputs( "\n", stdout );
...
/* TLimit = CALIB_SECONDS seconds. That is, a runtime of CALIB_SECONDS  */
...
/*            single/double precision operations per loop     */
...
/*        s = s + (D1+u*(D2+u*D3))/(w+u*(D1+u*(E2+u*E3)));	*/
       v += w;
       u = v * x;
       s += (D1+u*(D2+u*D3))/(w+u*(D1+u*(E2+u*E3)));
       }
       dtime(TimeArray);
       sa = TimeArray[1];
...
/*    s = s + u * ((((((A6*w-A5)*w+A4)*w-A3)*w+A2)*w+A1)*w+one);	*/
   s += u * ((((((A6*w-A5)*w+A4)*w-A3)*w+A2)*w+A1)*w+one);
   }
   dtime(TimeArray);
...
/*    s = s + w*(w*(w*(w*(w*(B6*w+B5)+B4)+B3)+B2)+B1)+one;	*/
   s += w*(w*(w*(w*(w*(B6*w+B5)+B4)+B3)+B2)+B1)+one;
   }
   dtime(TimeArray);
...
/*    s = s + v / (w*(w*(w*(w*(w*(B6*w+B5)+B4)+B3)+B2)+B1)+one);	*/
   s += v / (w*(w*(w*(w*(w*(B6*w+B5)+B4)+B3)+B2)+B1)+one);
   }
   dtime(TimeArray);
...
/*    s = s + v*v*u*((((((A6*w+A5)*w+A4)*w+A3)*w+A2)*w+A1)*w+one);	*/
   s += v*v*u*((((((A6*w+A5)*w+A4)*w+A3)*w+A2)*w+A1)*w+one);
   }
   dtime(TimeArray);
...
/**************************************************/ 
   ...
   printf("   MFLOPS(av)      = %10.4lf\n\n", (T[28]+T[30]+T[32]+T[34])/4 );
...
flop_t p[];
{
 ...
 p[2] = ( (flop_t)(tt.ticks + (tt.minutes * 60L * 50L)) ) / (flop_t)HZ;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 p[2] = (flop_t)(tms.tms_utime) / (flop_t)HZ;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 p[2] = (flop_t)(tms.proc_user_time) / (flop_t)HZ;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 p[2] = 60.0 * (flop_t)(tnow.ti_min);
 p[2] = p[2] + (flop_t)(tnow.ti_sec);
 p[2] = p[2] + (flop_t)(tnow.ti_hund)/(flop_t)HZ;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 p[2] = (flop_t)clock() / (flop_t)CLOCKS_PER_SEC;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 p[2] = (flop_t)tnow.tv_sec + (flop_t)tnow.tv_usec * 1.0e-6;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 p[2] = (flop_t)(rusage.tms_utime) * 1.0e-06;
 p[1] = p[2] - q;
...
flop_t p[];
{
 ...
 q = p[2];
 p[2] = (flop_t) (TimeNowHigh()) / (flop_t) CLK_TCK_HIGH;
 p[1] = p[2] - q;
